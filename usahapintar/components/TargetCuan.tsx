...
export default function TargetCuan() {
  ...
  const unitBulanan = labaPerUnit > 0 ? Math.ceil(targetBulanan / labaPerUnit) : 0;
  const unitHarian = unitBulanan > 0 ? Math.ceil(unitBulanan / 30) : 0;
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-[1.3fr_1fr]">
          <div className="rounded-md border-2 border-ink bg-paper p-6 shadow-[6px_6px_0_0_#1E2A1F]">
            <label className="block font-body text-sm font-semibold text-ink">Target keuntungan per bulan</label>
            <input type="number" min={0} value={targetBulanan} onChange={(e) => setTargetBulanan(Number(e.target.value) || 0)} className="mt-2 w-full rounded-sm border border-ink/20 bg-paper px-3 py-2 font-mono text-sm text-ink outline-none focus:border-forest" />
          ...
          <div className="rounded-md border-2 border-ink bg-paper shadow-[6px_6px_0_0_#1E2A1F]">
            ...
            <div className="bg-ledger-lines px-6 py-4 font-mono text-sm text-ink">
              <div className="flex justify-between py-1"><span className="text-muted">Target laba bulanan</span><span>{rupiah(targetBulanan)}</span></div>
            ...
            <div className="border-t-2 border-ink px-6 py-5"><p className="font-mono text-[11px] uppercase tracking-widest text-muted">Kesimpulan target</p><p className="mt-1 font-display text-xl font-semibold text-forest">{labaPerUnit > 0 ? `Sekitar ${unitHarian} produk per hari` : "Harga belum menghasilkan keuntungan"}</p><p className="mt-1 font-body text-xs text-muted">Dengan laba {rupiah(labaPerUnit)} per produk, target {rupiah(targetBulanan)} per bulan membutuhkan sekitar {unitBulanan} produk per bulan.</p><p className="mt-2 font-body text-xs text-muted">Estimasi laba bulanan: {labaPerUnit > 0 ? rupiah(estimasiLaba) : "-"}</p></div>
          </div>
        </div>
